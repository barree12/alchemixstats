...
export default class EarnVelo extends Component {
  ...
  parseResult(result){
    ...
    for(let i=0;i<result.data.length;i++){
      ...
      if(result.data[i].address === opAlUsd) {
        opAlUsdApr = result.data[i].apr;
        console.log("aaaa")
      }
      if(result.data[i].address === opAlEth) opAlEthApr = result.data[i].apr;
    }
    this.setState({ aprLoading: false, aprStable: stableApr, aprEth: ethApr, aprOpAlUsd: opAlUsdApr, aprOpAlEth: opAlEthApr })
    console.log(result)
  }
